...
test('third-person uniform colour blocking uses shared historic palette rows and remains visible from rear view',()=>{const s=fs.readFileSync(new URL('../../../apps/campus/src/player/student-avatar.ts',import.meta.url),'utf8');assert.match(s,/red=mat\(18\)/);assert.match(s,/navy=mat\(21\)/);assert.match(s,/skin=mat\(7/);assert.ok(s.includes("'shoulder-yoke'"));assert.ok(s.includes("'back-piping-'"));});
