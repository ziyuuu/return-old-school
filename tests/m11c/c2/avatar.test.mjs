...
test('student avatar uses shared materials and no external texture or school emblem asset',()=>{const s=fs.readFileSync(new URL('../../../apps/campus/src/player/student-avatar.ts',import.meta.url),'utf8');assert.ok(s.includes('surfaceMaterial'));assert.ok(!/TextureLoader|GLTFLoader|https?:\/\//.test(s));assert.ok(!/logo\.png|emblem/i.test(s));});
