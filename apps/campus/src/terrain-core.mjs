...
export function buildTerrainModel(l,spec){
 ...
 const exportData=()=>({version:spec.version,baseline:l.version,datum:spec.datum,surveyVerified:false,anchors,pads,stairs,profiles,unlocated:l.facilities.filter(f=>!f.position).map(f=>f.id),horizontal:horizontalSnapshot(l),note:l.patch02?'Patch02 owner-authorized gym-front/loop exception on an unchanged R4 archive. All exact elevations H.':'R4 horizontal coordinates/topology retained; vertical H overlay only. No survey or physical-character certification.'});
...
export function terrainChecks(l,spec,reference=l){
 ...
 for(const id of ['04','05','06','08','23','28'])put('SPORT_FLAT_'+id,Math.abs(t.anchors[id].floor-(spec.sportsDatum??0))<1e-8,'Flat sports pads, not rolling terrain');
