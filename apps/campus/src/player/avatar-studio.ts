...
document.getElementById('export')!.onclick=async()=>{const data=await exportGLB(),url=URL.createObjectURL(new Blob([data],{type:'model/gltf-binary'})),a=document.createElement('a');a.href=url;a.download='Yali_Student_C2_R3.glb';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
