"""R2: exercise actual geometry, not just JSON claims. Embedding is a restricted-host fallback."""
from pathlib import Path
import os,json,subprocess,time,hashlib
from playwright.sync_api import sync_playwright
ROOT=Path(__file__).resolve().parents[2]; OUT=ROOT/'qa/m10-r2';OUT.mkdir(parents=True,exist_ok=True)
embedded=os.getenv('M10_EMBEDDED_QA')=='1'
report={'version':'M1.0.1-R2','mode':'standalone embedded in Chromium about:blank' if embedded else 'HTTP production + standalone file','hardware_fps_certified':False,'checks':[],'errors':[],'screenshots':[]}
server=None
if not embedded:
...
try:
 with sync_playwright() as p:
  ...
  check('R2 boot and layout checks',page.evaluate('window.__YALI_M10__.version==="M1.0.1-R2" && window.__YALI_M10__.report.passed'))
  ...
  portal=page.evaluate('''()=>{const v=window.__YALI_M10__,fail=[];let count=0;
   for(const c of v.layout.connections){const a=[c.xStart-1,c.y+1.5,c.z],b=[c.xEnd+1,c.y+1.5,c.z];const hits=v.probeSegment(a,b);count++;if(hits.length)fail.push({id:c.id,hits});}
   for(const [id,a,b] of [['main-rear',[69,1.5,234],[69,1.5,230]],['library',[75,1.5,243],[75,1.5,247]],['gym-link',[-68,1.5,62],[-64,1.5,62]],['music-link',[-72,1.5,76],[-74,1.5,76]]]){const hits=v.probeSegment(a,b);count++;if(hits.length)fail.push({id,hits});}return{count,fail};}''')
  check('actual midpoint / rear-door / gym-link openings',not portal['fail'],portal)
  ...
  for view in ['overview','top','sports','teaching','courts-to-gym','canopy','rear-field','library-route','underpass','side-path']:
  ...
  page.locator('#roofs-check').check()
  page.locator('#footprints-check').check();check('footprint mode',page.evaluate('window.__YALI_M10__.getState().planOnly'))
  page.locator('#footprints-check').uncheck()
  ...
  dl.value.save_as(str(OUT/'exported-layout.json'));check('export is current R2',json.loads((OUT/'exported-layout.json').read_text())['version']=='M1.0.1-R2')
  ...
  report['browser']=browser.version;report['standalone_sha256']=hashlib.sha256(html.encode()).hexdigest();report['success']=True
  browser.close()
