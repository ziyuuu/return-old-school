...
const out=path.resolve(process.env.C2_QA_ROOT||'qa/m11c-c2/r2/release',group);fs.mkdirSync(out,{recursive:true});
const name=studio?'Yali_C2_R2_Character.html':'Yali_C2_R2_Viewer.html',viewer=path.resolve(process.env.C2_ARTIFACT_DIR||'artifacts/m11c-c2/r2',name);
...
try {
 ...
 if(studio){
  ...
  measurements=await page.evaluate(()=>{const s=window.__C2_STUDIO__,values=[];s.view('front');for(let i=0;i<68;i++){const t=performance.now();s.step(.0275,1.65);s.renderer.getContext().finish();if(i>=8)values.push(performance.now()-t);}const sorted=[...values].sort((a,b)=>a-b);return {scope:'Installed character, shared daylight and studio floor ONLY; not campus or real-phone FPS',warmup:8,samples:60,milliseconds:values,medianMs:sorted[29],p95Ms:sorted[56],avatar:s.avatar.state()};});
  ...
  const bytes=Buffer.from(data.split(',')[1],'base64');fs.writeFileSync(path.join(out,'Yali_Student_C2_R2.glb'),bytes);const json=JSON.parse(bytes.subarray(20,20+bytes.readUInt32LE(12)).toString());
  glb={file:'Yali_Student_C2_R2.glb',sha256:sha(bytes),bytes:bytes.length,skins:json.skins?.length,animations:json.animations?.map(a=>a.name)};
  ...
  check('approved-direction R2 rig is installed in real campus',state.avatar.artVersion==='C2.R2'&&state.avatar.uniform.logo==='YL',compact(state));check('balanced raster with original C1 collision',state.graphics.mode==='balanced',state.graphics);await shot('campus-gate-back');
  if(!mobile){
   ...
   await page.evaluate(()=>{const p=window.__YALI_C1__;p.spawn('gate');p.orbit(785,0);p.tick(0);});await shot('campus-gate-front');
   ...
   measurements=await page.evaluate(()=>{const p=window.__YALI_C1__,v=[];for(let i=0;i<68;i++){const t=performance.now();p.tick(1/60);if(i>=8)v.push(performance.now()-t);}const sorted=[...v].sort((a,b)=>a-b);return {scope:'CPU player update only at a stationary checkpoint; NOT GPU FPS',samples:60,warmup:8,milliseconds:v,medianMs:sorted[29],p95Ms:sorted[56],avatar:p.state().avatar};});
  }
 }
