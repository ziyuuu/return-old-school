...
def start(title,sub):return f'<svg xmlns="http://www.w3.org/2000/svg" width="1440" height="960" viewBox="0 0 1440 960"><rect width="1440" height="960" fill="#f4f4ee"/><style>text{{font-family:"Noto Sans CJK SC",Arial,sans-serif;fill:#203e38}}.line{{fill:none;stroke:#38594d;stroke-width:2}}</style><text x="50" y="56" font-size="30">{title}</text><text x="50" y="91" font-size="17">{sub}</text>'
...
X=lambda x:720+x*12;Z=lambda z:210+(7-z)*15
s+=text(50,140,'A｜整栋首层外壳轮廓：两端退让入口、前伸主体墙、中部轻弧')
for q in sorted(m['parts'],key=lambda q:q['role']!='slab'):
 ...
 else:s+=rect(X(x-w/2),Z(z+d/2),w*12,d*15,fill)
s+=line([(720,200),(720,490)])+text(735,185,'主轴 X=73（继承R2）',15)
...
captions={'r3-west-front.png':'图面左端入口正视｜门洞、门廊和台阶','r3-east-front.png':'图面右端入口正视｜门洞、门廊和台阶','r3-front.png':'整栋正面｜两端退入入口与中央轻弧','r3-overview.png':'整体斜前｜五层主体和一层门廊','r3-west.png':'左端入口｜门在正面，右侧高墙继续前伸','r3-east.png':'右端入口｜对应的内退门廊','r3-wall.png':'五层前伸墙｜不是门罩旁的一段短墙','r3-porch-plan.png':'首层剖看｜两端轮廓的凹凸关系','r3-arc.png':'中央功能区｜浅弧墙面与连续檐口','r3-arc-plan.png':'曲线俯看｜最前点与轴线保持','r3-axis.png':'校园四节点共轴','r3-section.png':'五层剖面回归','r2-terrace.png':'四楼大坪｜走廊有顶，大坪露天','r2-fifth.png':'五楼走廊','r2-bridge5.png':'第五层厕所连接','p04-courts.png':'P4篮球场抬高保留','p03-gate-out.png':'P3入口长坡保留','desktop-ui.png':'桌面操作界面','mobile.png':'手机尺寸显示'}
...
checklist=f'# B01 R3 回归清单\n\n工程 {"PASS"if ok else "INCOMPLETE"}；校友 REVIEW_PENDING。原583项全部保留，本轮107项（含36个故障注入）；总数{count}。\n\n'+''.join(f'- [{"x"if c["passed"]else " "}] {c["name"]}\n'for c in report['checks'])
