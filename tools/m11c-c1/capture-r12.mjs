...
const viewer=path.join(root,'artifacts/m11c-c1/Yali_C1_R1_2_Viewer.html'),manifest=JSON.parse(fs.readFileSync(path.join(root,'artifacts/m11c-c1/viewer-manifest-r12.json')));
...
const report={stage:'C1',version:'R1.2',status:'IMPLEMENTED / REVIEW_PENDING',phase,group,viewer:path.relative(root,viewer),viewerSHA256:manifest.sha256,sourceCommit:manifest.gitHead,viewport,startedAt:new Date().toISOString(),errors:[],warnings:[],externalRequests:[],screenshots:[],checks:[],passed:false};
...
const context=await browser.newContext({viewport,deviceScaleFactor:mobile?3:1,hasTouch:mobile,isMobile:mobile}),page=await context.newPage();page.setDefaultTimeout(180000);
...
page.on('pageerror',e=>report.errors.push(String(e)));page.on('console',m=>{if(m.type()==='error')report.errors.push(m.text());if(m.type()==='warning')report.warnings.push(m.text());});page.on('request',r=>{if(/^https?:/.test(r.url())){if(r.isNavigationRequest()&&process.env.C1_VIEWER_ORIGIN&&r.url().startsWith(process.env.C1_VIEWER_ORIGIN+'/'))report.documentRequest=r.url();else report.externalRequests.push(r.url());}});
...
async function capture(name){
 ...
 try{await page.screenshot({path:path.join(out,file),scale:'css',timeout:20000});}
...
try{
 ...
 const url=process.env.C1_VIEWER_ORIGIN?process.env.C1_VIEWER_ORIGIN+'/'+path.relative(root,viewer):pathToFileURL(viewer).href;await page.goto(url+q,{waitUntil:'load',timeout:600000});
 ...
 check('Lossless packing leaves triangle count unchanged',report.initialization.lighting.vertexPacking.trianglesBefore===report.initialization.lighting.vertexPacking.trianglesAfter,report.initialization.lighting.vertexPacking);
 check('Lossless packing lowers geometry buffer bytes',report.initialization.lighting.vertexPacking.bytesAfter<report.initialization.lighting.vertexPacking.bytesBefore);
 ...
 if(group==='lane'){
  ...
  check('Walled lane to residential fork with region transitions',path.reason==='complete'&&path.end.motor.feet[0]>128.5&&path.end.collision.regionRevisions>=5,path.end.motor);await capture('03-residential-fork');check('Physics transitions reused exact resident meshes',path.end.collision.cache.reused>0,path.end.collision.cache);
 }
 if(group==='terrain'){
 ...
 if(mobile){
  ...
  await page.locator('#c1-menu').click();await capture('02-mobile-help');
  check('Phone starts balanced at DPR at most one',(await state()).graphics.mode==='balanced'&&(await state()).graphics.actualPixelRatio<=1);
  await page.locator('#c1-quality').selectOption('detail');let quality=(await state()).graphics;
  check('Full-quality raster option restores original DPR',quality.actualPixelRatio===1.5&&quality.shadowResolution===2048,quality);
  await page.locator('#c1-quality').selectOption('balanced');quality=(await state()).graphics;
  check('Balanced raster restored without reload',quality.actualPixelRatio===1&&quality.shadowResolution===1024,quality);
  await page.locator('#c1-spawn').selectOption('lane');await page.locator('#c1-reset').click();await zero();await capture('03-mobile-lane');
  await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[touch(x,y)]});await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[touch(x,y-35)]});s=await state();check('Cancellation test starts from active touch motion',Math.hypot(s.input.x,s.input.z)>.5,s.input);await cdp.send('Input.dispatchTouchEvent',{type:'touchCancel',touchPoints:[]});s=await state();check('Real pointer cancellation cannot leave motion stuck',Math.hypot(s.input.x,s.input.z)===0,s.input);
  report.mobile=await page.evaluate(()=>({width:innerWidth,scrollWidth:document.documentElement.scrollWidth,shadow:window.__YALI_RENDER__.state().shadowResolution}));check('Fresh 390x844 branch and no overflow',report.mobile.width===390&&report.mobile.scrollWidth===390&&report.mobile.shadow===1024,report.mobile);await cdp.detach();
 }
 if(group==='fix'){
  await zero();const canvas=page.locator('#viewport canvas');await canvas.focus();const start=(await state()).motor.feet;
  await page.keyboard.down('d');await tick(70);await page.keyboard.up('d');await zero();let s=await state();check('Camera-relative D moves screen-right at yaw 0',s.motor.feet[0]<start[0]-1.3,{start,after:s.motor.feet,input:s.input});await capture('01-right-control');
  await page.keyboard.press('r');await zero();const reset=(await state()).motor.feet;await page.keyboard.down('a');await tick(70);await page.keyboard.up('a');await zero();s=await state();check('Camera-relative A moves screen-left at yaw 0',s.motor.feet[0]>reset[0]+1.3,{reset,after:s.motor.feet});
  await page.keyboard.press('r');await zero();const uphill=await page.evaluate(()=>window.__YALI_C1__.walkTo([[0,12],[0,30],[0,44]],false));report.slopeUphill=uphill;const upSamples=uphill.trace.filter(x=>x.slopeDegrees>2.2);check('Actual main entrance uphill reduces target speed',uphill.reason==='complete'&&upSamples.length>0&&upSamples.some(x=>x.slopeSpeedMultiplier<.99),upSamples.slice(-5));await capture('02-uphill-speed');
  const downhill=await page.evaluate(()=>window.__YALI_C1__.walkTo([[0,30],[0,12],[0,2]],false));report.slopeDownhill=downhill;const downSamples=downhill.trace.filter(x=>x.slopeDegrees<-2.2);check('Actual main entrance downhill gains only bounded speed',downhill.reason==='complete'&&downSamples.length>0&&downSamples.some(x=>x.slopeSpeedMultiplier>1)&&downSamples.every(x=>x.slopeSpeedMultiplier<=1.120001),downSamples.slice(-5));await capture('03-downhill-speed');
 }
