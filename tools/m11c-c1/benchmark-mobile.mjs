...
try{
 for(const [version,file]of[['R1.1','Yali_C1_R1_1_Viewer.html'],['R1.2','Yali_C1_R1_2_Viewer.html']]){
  ...
  // Compile/warm through the actual render entrypoint; retain all geometry and lights.
  for(let i=0;i<68;i++){
  ...
  const sorted=[...session.valuesMs].sort((a,b)=>a-b);session.medianMs=(sorted[29]+sorted[30])/2;session.p95Ms=sorted[56];
  await p.screenshot({path:path.join(out,version+'-gate.png'),scale:'css',timeout:90000});
  session.screenshot=version+'-gate.png';session.screenshotSHA256=createHash('sha256').update(fs.readFileSync(path.join(out,session.screenshot))).digest('hex');
 ...
 const [before,after]=report.sessions;report.comparison={synchronousFrameCostRatio:after.medianMs/before.medianMs,routeCpuCostRatio:after.route.milliseconds/before.route.milliseconds,warning:'Same hosted software renderer; includes changed raster budget; do not infer phone frame rate.'};
 report.passed=true;
