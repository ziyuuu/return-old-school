...
try{
 ...
 if(mobile){
  ...
  // Use a real active browser pointer: synthetic dispatchEvent(pointerdown) cannot
  // legally acquire pointer capture and was itself the iteration-01 JS error.
  await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[touch(x,y)]});
  await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[touch(x,y-35)]});
  s=await state();check('Cancellation test starts from active touch motion',Math.hypot(s.input.x,s.input.z)>.5,s.input);
  await cdp.send('Input.dispatchTouchEvent',{type:'touchCancel',touchPoints:[]});
  s=await state();check('Real pointer cancellation cannot leave motion stuck',Math.hypot(s.input.x,s.input.z)===0,s.input);
