...
assert.ok(['b05-gate-close','b05-hoop','b05-overview','b05-stone','b05-tree'].includes(view));
const out=path.join(root,'qa/m11b-b05',phase,'performance-resume',view);await fs.mkdir(out,{recursive:true});
const viewer=process.env.B05_VIEWER_FILE||'artifacts/m11b-b05/Yali_B05_R1_1_Viewer.html',hash=b=>createHash('sha256').update(b).digest('hex'),html=await fs.readFile(path.join(root,viewer));
const manifest=JSON.parse(await fs.readFile(path.join(root,process.env.B05_MANIFEST_FILE||'artifacts/m11b-b05/viewer-manifest.json'),'utf8'));assert.equal(hash(html),manifest.sha256);
