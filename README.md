...
当前版本见 [R4修订与验收](docs/m10/revision-r4.md) 和 [回归清单](docs/m10/regression-r4.md)。当前QA为qa/m10-r4；以下旧记录为历史交付。
...
本轮按校友照片与批注修复7项：风雨跑道顶棚、中部逐层连接、后缘橡胶跑道与沙坑、侧门小路、主楼后方至图书馆通路、体育馆放大并连接音乐楼、26号旧厕重定位缩小。
...
- 主楼图面左侧独栋厕所：每层连接、相向立面中部接入、门朝教学楼；两楼间地面可穿行至图书馆。草图按通道分叉到男女两翼理解，不套用到池畔厕所。
- 篮球场：3×2 六片，与有顶棚的相邻直跑道平行且紧贴，中间不插道路或绿化。
- 直跑道夹在体育辅助用房与篮球场之间。项目编号05，08表示田径环道；编号不是历史正式名称。
- 体育辅助用房28：低矮单层，健身/锻炼、体育教师办公室、器材空间是功能类别，不等于三栋楼。
- 音乐楼24：四层独立体块，跑道旁、体育馆后方，与03有实际连接，不与28合并。
- 音乐楼、WC、泳池位于条形空间同一端；池畔厕所26在泳池图面左侧。
- 主席台位于操场图面左侧，沙坑位于操场靠主楼一端的后缘橡胶直段右端；校门参考视图右侧有校名石。
